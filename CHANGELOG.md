## Unreleased
* **FEATURE** Add `LineTouchData.crosshair` (`TouchCrosshair`) to draw a full plot-area-height vertical line at the touched spot's x-position while the user is touching the chart. Composes with `getTouchedSpotIndicator` and `extraLinesData`.
* **FEATURE** Add `TouchCrosshair.followDuration` so the crosshair can glide smoothly between spots as the finger moves, instead of snapping. `Duration.zero` (default) preserves snap behavior.
